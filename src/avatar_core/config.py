...
def load_secrets(explicit: Path | None = None) -> dict[str, str]:
    ...
    #
    # Список белый, а не чёрный, и это стоило нам живого часа: TELEGRAM_
    # забыли добавить вместе с телеграм-ботом, и токен из EnvironmentFile
    # просто не доезжал. Локально всё работало — там он лежал в файле
    # секретов и проходил по ветке «ключ уже известен», — а на сервере
    # секреты приходят из окружения юнита, и бот молча оставался без токена.
    # Добавляете новый источник секретов — добавьте сюда его префикс.
    prefixes = ("H3_", "KANDINSKY_", "MAX_", "MINIAPP_", "TELEGRAM_")
    values.update({
